...
const SquareLightGame = () => {
    ...
    const generateRandomSquare = () => {
        ...
        const newGrid = Array(6).fill(Array(6).fill(false)).map((r,i)=>
    ...
    const handleSquareClick = (row,col) =>{
        ...
        if(isPlay && isLightSquare && isLightSquare[0]===row && isLightSquare[1]===col) {
            clickRef.current = clickRef.current+1;
        console.log(clickRef.current)
            const endTime = Date.now();
            const responseTime = endTime -startTime;
            setResponsesTimes(prevValue=>[...prevValue,{mouseClick: clickRef.current,reactionTime:responseTime}])
            setStartTime(null)
  ...
  return (
    ...
    {/* header */}
      ...
      <input type="number" maxLength={5} minLength={0} placeholder='Enter seconds between 1 and 5' onChange={(e)=>setSeconds(e.target.value)} />
     ...
     {/* content */}
     <div style={{display:'grid',gridTemplateColumns:'repeat(6,50px)',gridTemplateRow:'repeat(6,50px)',margin:'20px'}}>
        {grid?.map((row,rowIndex)=>(
            <div key={rowIndex}>
                {row?.map((col,colIndex)=>(
                    <div key={colIndex} style={{width:'50px',height:'50px',border:'1px solid green',
                    display:'flex',
